...
function calcularPrecoFinal(precoMedio, descontoFixo, qtdDefeitos) {
  const descontoInicial = precoMedio * 0.15; // 15% de desconto fixo
  const descontoPorDefeitos = precoMedio * (0.05 * qtdDefeitos); // 5% por peça com defeito
...
function atualizarDefeitos(precoMedio, descontoFixo) {
  const precoFinal = calcularPrecoFinal(precoMedio, descontoFixo, qtdDefeitos);
...
document.getElementById("add-defeito").addEventListener("click", () => {
  ...
  const precoMedio = parseFloat(
    document.getElementById("revenda").querySelector("p").textContent
      .replace("Preço médio do veículo: R$", "")
      .replace(/\./g, "")
      .replace(",", ".")
  );
  const descontoFixo = 5000;
...
function renderVeiculo(data) {
  ...
  const descontoFixo = 5000; // Desconto fixo para revenda
...
document.addEventListener("DOMContentLoaded", function() {
    const removerPeçasBtn = document.getElementById("remover-peças");
    const quantidadeElem = document.getElementById("quantidade");
    let quantidade = 0;
  
    // Função para atualizar a quantidade no HTML
    function atualizarQuantidade() {
      quantidadeElem.textContent = quantidade;
    }
  
    // Evento para remover todas as peças com defeito
    removerPeçasBtn.addEventListener("click", function() {
      quantidade = 0; // Resetando a quantidade de peças com defeito
      atualizarQuantidade(); // Atualizando o contador para 0
    });
  });
